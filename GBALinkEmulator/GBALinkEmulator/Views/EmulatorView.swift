        ...
        return ZStack {
            ...
            .overlay(alignment: .topLeading) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text(String(format: "FPS: %.1f", viewModel.currentFPS))
...
final class EmulatorViewModel: ObservableObject {
    ...
    func start(rom: ROMFile, linkSession: LinkCableSession) {
        ...
        do {
            print("[EmulatorVM] Starting real mGBA emulation")
            try core.loadROM(at: url)
            print("[EmulatorVM] ✅ REAL MGBA MODE - Game is loading")
            core.reset()
    ...
    private func startFPSUpdates() {
        fpsUpdateTimer?.invalidate()
        fpsUpdateTimer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { [weak self] _ in
            // Timer runs on main thread by default when created on main thread
            guard let self = self else { return }
            self.currentFPS = Double(self.core.renderedFrameCount)
        }
    }
