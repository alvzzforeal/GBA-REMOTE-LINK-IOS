...
struct EmulatorView: View {
    ...
    private func screenArea(in geo: GeometryProxy, safeInsets: EdgeInsets) -> some View {
        ...
        // Must account for top safe area (notch / Dynamic Island) so screen doesn't overflow
        let reservedH = topBarH + controlsH + safeInsets.top + safeInsets.bottom + verticalPadding

        let availableW = geo.size.width - 24
        let availableH = max(geo.size.height - reservedH, 100)

        // GBA native: 240x160 (3:2) — fill available space
        let scaleByW = availableW / 240
        let scaleByH = availableH / 160
        let scale    = min(scaleByW, scaleByH)
        let screenW  = (240 * scale).rounded()
        let screenH  = (160 * scale).rounded()
        ...
        return ZStack {
            ...
            .overlay(alignment: .topLeading) {
                VStack(alignment: .leading, spacing: 3) {
                    Text(viewModel.coreMode)
                        .font(.system(size: 9, weight: .bold, design: .monospaced))
                        .foregroundColor(.black)
                        .padding(.horizontal, 6)
                        .padding(.vertical, 3)
                        .background(viewModel.isStubMode ? Color.yellow : Color.green)
                        .clipShape(Capsule())
    ...
    // Controls height: scales with both width and height of device.
    // iPhone 13 mini (375pt) → ~170pt, iPhone 15 (390pt) → ~185pt, Pro Max (430pt) → ~200pt
    private func controlsHeight(for size: CGSize) -> CGFloat {
        let wScale = size.width / 390          // 1.0 baseline = iPhone 15
        let base: CGFloat = 185
        let scaled = (base * wScale).rounded()
        // Clamp: never more than 42% of screen height, never less than 160pt
        return min(max(scaled, 160), size.height * 0.42)
    }
...
final class EmulatorViewModel: ObservableObject {
    ...
    @Published var coreMode: String = "LOADING CORE"
    @Published var isStubMode: Bool = true
    ...
    func start(rom: ROMFile, linkSession: LinkCableSession) {
        ...
        core.frameCallback = { [weak self] image in
            ...
            self?.coreMode = self?.core.coreModeDescription ?? "UNKNOWN CORE"
            self?.isStubMode = self?.core.isStubMode ?? true
        }
        linkSession.onSerialData = { bytes in
            print("[Link] Received \(bytes.count) serial bytes")
        }
        guard let url = rom.resolveURL() else {
            loadError = "ROM file not found. Try re-importing it from the Files app."
            print("[EmulatorVM] resolveURL() returned nil for \(rom.fileName)")
            return
        }
        do {
            try core.loadROM(at: url)
            coreMode = core.coreModeDescription
            isStubMode = core.isStubMode
            if core.isStubMode {
                print("[EmulatorVM] WARNING: STUB MODE active. Reason: \(core.lastErrorMessage)")
            } else {
                print("[EmulatorVM] REAL MGBA MODE active")
            }
            core.reset()
            core.startDisplayLink()
        } catch {
            loadError = "Failed to load ROM: \(error.localizedDescription)"
