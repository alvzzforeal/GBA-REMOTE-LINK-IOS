...
- (instancetype)init {
    ...
    NSLog(@"[GBABridge] Compiled core mode: %@", self.coreModeDescription);
    return self;
}
...
- (BOOL)loadROM:(NSString *)path error:(NSError **)outError {
    ...
    _loaded = YES;
    _lastErrorMessage = @"";
    NSLog(@"[GBABridge-mGBA] ROM loaded successfully: %@", path.lastPathComponent);
    return YES;
    ...
    _loaded = YES;
    _lastErrorMessage = @"STUB MODE - mGBA headers/library were not linked at compile time.";
    _frameCount = 0;
...
- (void)runFrame {
    ...
    if (_core && _loaded) {
        ...
        if ((_frameCount % 120) == 0) {
            NSLog(@"[GBABridge-mGBA] runFrame OK, frames=%d", _frameCount);
        }
    } else if (!_loaded) {
        if ((_frameCount % 120) == 0) NSLog(@"[GBABridge-mGBA] runFrame called but ROM is not loaded");
    }
#else
    if (!_loaded) return;
...
#else
    return @"STUB MODE - mGBA not linked";
#endif
}
