...
final class GBAEmulatorCore: GBACoreProtocol {
    ...
    func loadROM(at url: URL) throws {
        print("[GBAEmulatorCore] Loading ROM: \(url.path)")
        print("[GBAEmulatorCore] Core mode before load: \(bridge.coreModeDescription)")
        try bridge.loadROM(atPath: url.path)
        print("[GBAEmulatorCore] Core mode after load: \(bridge.coreModeDescription)")
        print("[GBAEmulatorCore] ROM loaded: \(url.lastPathComponent)")
        if bridge.isStubMode {
            print("[GBAEmulatorCore] WARNING: STUB MODE. Game will NOT emulate. \(bridge.lastErrorMessage)")
        }
        setupAudio()
    }
    ...
    func runFrame() {
        ...
        bridge.runFrame()
        let image = makeImage()
        frameCallback?(image)   // CADisplayLink already fires on main thread
    ...
    /// Convert mGBA's ARGB8888 video buffer to a CGImage.
    private func makeImage() -> CGImage? {
        guard let rawPtr = bridge.videoBuffer else {
            print("[GBAEmulatorCore] videoBuffer is nil")
            return nil
        }
