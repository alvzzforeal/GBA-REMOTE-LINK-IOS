...
// MARK: - GBA Input State

struct GBAInputState: Equatable {
    var a:      Bool = false
    var b:      Bool = false
    ...
    /// Bit-mask compatible with mGBA's GBAKey enum order
    var keyMask: UInt16 {
        var mask: UInt16 = 0
        if a      { mask |= GBAKeyMask.A.rawValue }
        if b      { mask |= GBAKeyMask.B.rawValue }
        if select { mask |= GBAKeyMask.select.rawValue }
        if start  { mask |= GBAKeyMask.start.rawValue }
        if right  { mask |= GBAKeyMask.right.rawValue }
        if left   { mask |= GBAKeyMask.left.rawValue }
        if up     { mask |= GBAKeyMask.up.rawValue }
        if down   { mask |= GBAKeyMask.down.rawValue }
        if r      { mask |= GBAKeyMask.R.rawValue }
        if l      { mask |= GBAKeyMask.L.rawValue }
        return mask
    }
}
...
final class GBAEmulatorCore: GBACoreProtocol {
    ...
    func loadROM(at url: URL) throws {
        var nsError: NSError?
        let ok = bridge.loadROM(atPath: url.path, error: &nsError)
        guard ok else {
